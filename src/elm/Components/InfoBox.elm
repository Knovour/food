module Components.InfoBox exposing (ColorPalette, colorBlocks, colorPalettes, infoBox)

import Html exposing (Html, a, div, h5, li, p, span, text, ul)
    ...
    (\{ name, color, tag } ->
      ...
      in
        a [ class "slot", href colorLink, target "_blank" ]
          [ div [ class "color-box", style "background" color, style "color" fontColor ]
            [ div [ class "info" ]
