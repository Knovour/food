...
classes isCurrentTag noResult =
  let
    class1 = if isCurrentTag then " current" else ""
...
tags { group, tag } foodList =
  let
    toggle = if tag == "標籤" then " _hide" else ""
    tagList = List.map (\name ->
      let
        value = getDictValue name foodList
      in
        div [ classes (name == group) ((List.length value) == 0), onClick (ActionMsg <| Group name) ] [ text name ]
    ) foodTypes
  in
    div [ class ("tags-block" ++ toggle) ]
